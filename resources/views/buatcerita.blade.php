...
<div class="flex justify-center my-8 sm:my-10 px-4 lg:px-10 xl:px-20">
    <form method="post" action="/story" enctype="multipart/form-data" class="w-full max-w-2xl">
        @csrf
        <div class="bg-white dark:bg-dark-card p-5 sm:p-7 rounded-lg shadow-xl border border-border-default dark:border-dark-border/50 flex flex-col space-y-5">
            ...
            <div>
                <label for="cover" class="block text-sm font-medium text-primary-text dark:text-dark-text mb-1">Tambahkan sampul buku</label>
                <img class="img-preview mb-3 rounded-md" style="display:none; max-width:120px; max-height: 170px; overflow: hidden;">
                <input type="file" id="cover" name="cover" onchange="previewImage()" 
                    class="block w-full text-sm text-primary-text dark:text-dark-text file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary-color file:text-white hover:file:bg-cta-hover file:transition file:duration-200 dark:file:bg-primary-color dark:file:text-white dark:file:hover:bg-cta-hover cursor-pointer border border-border-default dark:border-dark-border/50 rounded-lg p-2 bg-gray-100 dark:bg-dark-border/50">
