...
<body class="bg-gray-50 dark:bg-dark-bg font-sans min-h-screen transition-colors duration-300"> 
    ...
    <main class="container mx-auto px-4 sm:px-8 py-6"> 
        <div class="buat flex justify-center mb-8">
            <a href="/perpus/create" class="btn flex items-center justify-center w-40 h-10 rounded-full bg-primary-color hover:bg-primary-hover text-white text-base font-bold transition duration-200 shadow-md hover:shadow-lg space-x-3">
                <i class="fa-solid fa-plus text-sm"></i>
                <span>Buat Koleksi</span>
            </a>
        </div>
        <div class="isi flex flex-col mx-auto max-w-4xl space-y-4 sm:space-y-6">
          ...
          <div class="koleksi flex justify-between items-center bg-white dark:bg-dark-card p-4 sm:p-5 rounded-xl shadow-lg border border-border-default dark:border-dark-border/50 transition-all duration-300 hover:shadow-xl hover:border-primary-color dark:hover:border-primary-color">
                <a href="/perpus/{{ $bookmark->slug }}" class="link text-primary-text dark:text-dark-text w-full">
                    <div>
                        <p class="text-2xl sm:text-3xl font-bold mb-1">{{ $bookmark->name }}</p>
                        <p class="tanggal text-sm font-medium text-secondary-text dark:text-dark-secondary-text">
                          {{ $bookmark->created_at->format('d F Y') }}
                        </p>
                    </div>
                </a>                
                <div class="button flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
                  <a href="/perpus/{{ $bookmark->slug }}/edit">
                    <button class="bg-primary-color dark:bg-primary-color text-white hover:bg-primary-hover dark:hover:bg-primary-hover p-2 w-8 h-8 rounded-md text-sm transition duration-200 flex items-center justify-center" aria-label="Edit Koleksi">
                        <i class="fa-solid fa-pencil"></i>
                    </button>
                  </a>
                  <form action="/perpus/{{ $bookmark->slug }}" method="post">
                    @method('delete')
                    @csrf
                    <button class="bg-primary-color dark:bg-primary-color text-white hover:bg-primary-hover dark:hover:bg-primary-hover p-2 w-8 h-8 rounded-md text-sm transition duration-200 flex items-center justify-center" aria-label="Hapus Koleksi"
                    onclick="return confirm('Yakin ingin menghapus koleksi?')">
